                ...
                <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all w-full max-w-2xl">
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <div>
                                    <h3 class="text-base font-semibold text-white">
                                        Résultats d'import des mappings
                                    </h3>
                                    <p class="text-xs text-purple-100">
                                        Correspondances comptables • {{ $summary['timestamp'] }}
                                    </p>
                                </div>
                            </div>
                            <button wire:click="closeResults" class="text-white hover:text-purple-200">
                    ...
                    <div class="px-4 py-3 border-b bg-gray-50">
                        <div class="grid grid-cols-3 gap-2">
                            <div class="bg-white p-2 rounded shadow-sm border-l-4 border-purple-500">
                                <p class="text-[10px] text-gray-500 uppercase">Total lignes</p>
                        ...
                        @if(count($allMessages) > 0)
                            <div class="space-y-1.5">
                                @foreach($allMessages as $index => $message)
                                    @php
                                        $isError = str_contains($message, '❌') || str_contains($message, 'introuvable') || str_contains($message, 'Erreur');
                                        $isSuccess = str_contains($message, '✓') || str_contains($message, 'succès') || str_contains($message, 'créé');
                                    @endphp
                                    <div class="p-2 rounded {{ 
                                        ...
                                        <div class="flex items-start">
                                            ...
                                            <div class="flex-1 text-xs {{ $isError ? 'text-red-700' : ($isSuccess ? 'text-green-700' : 'text-gray-700') }}">
                                                {!! nl2br(e($message)) !!}
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="px-4 py-2 bg-gray-50 border-t flex items-center justify-between">
                        ...
                        <div class="flex gap-2">
                            <button wire:click="downloadPdf"
                                    class="inline-flex items-center px-3 py-1.5 bg-purple-600 text-white text-xs font-medium rounded hover:bg-purple-700 transition shadow-sm">

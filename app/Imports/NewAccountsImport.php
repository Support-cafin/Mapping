...
class NewAccountsImport implements 
    ToCollection, 
    WithHeadingRow, 
    WithStartRow, 
    WithBatchInserts, 
    ...
    SkipsOnFailure
{
    ...
    protected $columnMapping = [
        'code' => ['code', 'codecompte', 'cod', 'account_code'],
        'intitule' => ['intitule', 'intitulé', 'libelle', 'libellé', 'label', 'nom', 'name', 'description'],
    ...
    private function loadExistingCodes()
    {
        ...
        $this->existingCodesCache = Cache::remember(
            "new_accounts_codes_{$this->entrepriseId}_{$exoId}",
            3600, // 1 heure
            function () use ($exoId) {
                return NewAccount::where('entreprise_id', $this->entrepriseId)
    ...
    {
        Log::info('=== DÉBUT IMPORT NOUVEAUX COMPTES ===', [
    ...
    private function insertAccounts($accountsData)
    {
        ...
        if (!$exoId) {
            $this->importErrors[] = "❌ Aucun exercice actif trouvé";
            return;
        }
    ...
    private function resolveParentId($parentCode, $line, $childCode)
    {
        ...
        // Rechercher en base
        $parent = NewAccount::where('entreprise_id', $this->entrepriseId)
            ->where('exercice_id', $exoId)
    ...
    {
        try {
            Cache::forget("new_accounts_codes_{$this->entrepriseId}_{$this->getExerciceId()}");
            
            if (method_exists(Cache::getStore(), 'tags')) {
                Cache::tags(['accounts', 'entreprise_' . $this->entrepriseId])->flush();
            }
        } catch (\Exception $e) {

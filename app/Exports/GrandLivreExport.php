...
class GrandLivreExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithColumnWidths, WithTitle, WithEvents
{
    ...
    protected function getRecapCollection()
    {
        ...
        $newAccounts = \App\Models\NewAccount::where('entreprise_id', $this->entreprise->id)->where('exercice_id', $exo->id)
            ->whereHas('mappings')
            ->orderBy('code')
            ->get();
        ...
        foreach ($newAccounts as $newAccount) {
            $oldAccountIds = \App\Models\AccountMapping::where('entreprise_id', $this->entreprise->id)->where('exercice_id', $exo->id)
